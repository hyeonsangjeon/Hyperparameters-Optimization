...
커밋된 `quick` 모드 실행 결과 덕분에 방문자가 셀을 실행하지 않아도 GitHub에서
모든 결과 표와 차트를 바로 볼 수 있습니다.
...
├── tools/execute_notebooks.py        # 동기화된 출력과 차트 게시
...
`execute_notebooks.py`는 한국어 원본을 `quick` 모드로 한 번 실행한 뒤, 바이트 단위로
동일한 코드 출력을 영문 노트북에도 복제합니다. 노트북 코드, `src/hpo_lab/`,
`pyproject.toml`, `uv.lock`을 포함한 runtime fingerprint로 오래되거나 삭제된 출력은
테스트와 CI에서 실패합니다. 코드 셀이 바뀌지 않은 소스 재생성은 게시된 출력을
그대로 보존합니다.
